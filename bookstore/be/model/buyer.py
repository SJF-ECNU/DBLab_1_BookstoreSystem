...
class Buyer(db_conn.DBConn):
    ...
    def payment(self, user_id: str, password: str, order_id: str) -> (int, str): # type: ignore
        try:
            ...
            self.db.new_order.update_one(
                {"order_id": order_id},
                {
                    "$set": {
                        "is_paid": True,
                        "status": "shipped" if self.db.new_order.find_one({"order_id": order_id})["is_shipped"] else "paid"
                    }
                }
            )            
